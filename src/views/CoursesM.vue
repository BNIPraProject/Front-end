<template>
  <v-app>
    <v-card>
      <v-tabs background-color="light-blue darken-4" center-active dark>
        <v-tab>Semua Course</v-tab>
        <v-tab route :to="courses">Course Saya</v-tab>
      </v-tabs>
    </v-card>
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      this.editedIndex = this.coursesm.indexOf(item);
    ...
    deleteItemConfirm() {
      this.coursesm.splice(this.editedIndex, 1);
      this.closeDelete();
    },
    ...
    save() {
      if (this.editedIndex > -1) {
        Object.assign(this.coursesm[this.editedIndex], this.editedItem);
      } else {
        this.coursesm.push(this.editedItem);
      }
      this.close();
    },
  },
